...
- `git status --short`, in a repo: uncommitted changes to this task's files, left by a session that stopped without a handoff.
...
2. Mark it: `status: doing`. When the part reads decided or done, set its status line to `building` and its node class to `:::building`. A task that was already doing: read what Delivered and the last Review pass say and carry on from there; the findings are the work list. When the newest entry holds an `Open question:` the human has not answered in this session, ask it and wait; the answer goes into the next entry. Uncommitted changes to this task's files: say so, check them against Delivers and the findings, and carry on from them. Done when the task reads doing and the part reads building.
...
5. When you cannot finish without an answer from the human, stop early (step 6), with an `Open question:` line in that entry and `Next: /build-it <id>`. Otherwise write Delivered as a dated entry (`### YYYY-MM-DD`) below any earlier ones: what was made, where it lives, how it was verified (what you ran or looked at, and what you saw), which boxes you could not check and why, and any decision or term you wrote. Name every delivered file by its path from the project root. End the entry with `Version: <checksum> (<n> files)`, where the checksum is `shasum -a 256 <files> | shasum -a 256 | cut -c1-12` run from the project root over those paths, written without `./` and in byte order (`LC_ALL=C sort`), because another order or spelling gives another checksum; when the delivery is not a file (a sent email, a setting in a tool), write `Version: none, <what to look at>` instead. Then set `status: review`. Done when Delivered reads as claims a reviewer can check and the task reads review.
...
7. Save as the project's `Saving work` section says: the files you made, the task file, `MAP.md` (or the file in `map/` that holds the part), and any decision or glossary entry you wrote; message `build <id>: <what was made>`, or `build <id>: stopped, <why>` after an early stop. Done when they are saved, or you have said why they are not.
8. Print `Next: /review-it <id>` as the last line.

## Output
- The delivered thing, where Delivered says.
- The task: status review, boxes ticked, a dated Delivered entry ending with its `Version:` line. After an early stop: status doing, and a dated entry saying where it stopped.
