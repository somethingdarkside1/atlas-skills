---
name: map-it
description: Check the map, redraw the map and glossary diagrams from their sections, set derived statuses, and report drift; or split a sketched part into its own file.
...
5. Bare: redraw the glossary diagram. A node per term that has an edge; an edge from a term to each term its definition names. Done when every node is an entry and every entry that names another term has its edge.
6. Bare: report what the sections could not fix, one line each: more than nine sections in one file (name the part to split); a term named in a definition or a `_Not_` line that has no entry. Print `No drift.` when there is nothing. Done when the report is printed.
7. With a part id: split it. Only a sketched part with no brief can be split; for any other, print `Splitting a part that has a brief is not supported yet.` and `Next: /atlas`, and stop. Create `map/<part>.md` with `part: <part>` frontmatter, the part's body, one paragraph, its own diagram, and one `##` section per sub-part with status sketched, `Needs:`, and open questions. When the sub-parts were not given, ask for them in one round in the interview format (title, why now, lettered options of one line, `➡️`). Sub-part ids must be unique across the whole map. In `MAP.md` leave the part's status line and `See [map/<part>.md](map/<part>.md).` Done when the part reads the same in both files and every id is unique.
8. Save as the project's `Saving work` section says: only the files you changed; message `map: regenerated` or `map: split <part>`. Done when they are saved, or you have said why they are not.
9. Print `Next: /atlas` as the last line.
