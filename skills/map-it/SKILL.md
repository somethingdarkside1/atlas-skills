...
2. Bare: regenerate the map diagram from the sections. `flowchart LR`; the four classDef lines from the format comment; one node per `##` section with id equal to the heading lowercased with dashes, label equal to the heading, class equal to the status; one unlabelled edge per `Needs:` id, from the needed part to the part that needs it; no subgraphs. Do the same in each `map/<part>.md` from its own sections. Done when the block in each file is replaced and renders.
3. Bare: redraw the glossary diagram. A node per term that has an edge; an edge from a term to each term its definition names. Done when every node is an entry and every entry that names another term has its edge.
...
5. With a part id: split it. Create `map/<part>.md` with `part: <part>` frontmatter, the part's body, one paragraph, its own diagram, and one `##` section per sub-part with status sketched, `Needs:`, and open questions. When the sub-parts were not given, ask for them in one round in the interview format (title, why now, lettered options with costs, `➡️`). Sub-part ids must be unique across the whole map. In `MAP.md` leave the part's status line and `See [map/<part>.md](map/<part>.md).`; `/atlas` recomputes that status from the sub-parts. Done when the part reads the same in both files and every id is unique.
6. Commit when the folder is a git repo: only the files you changed; message `map: regenerated` or `map: split <part>`; on a rejected hook, print its one line and stop. Done when `git status` shows none of them.
