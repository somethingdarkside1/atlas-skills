...
5. **Evidence has no light setting.** `core-model/01` needs about 650 words of Delivered and Review, with revision hashes, to record that nine decisions were accepted. If that is the model task, a brand designer will not keep it up. [0019](../decisions/0019-acceptance-follows-output-and-scope.md) is right that a review must name what it looked at. One `Version:` line does that.
...
## Copied into
Nothing durable. The choices above are proposals until Vitali answers them. A visual guide built from this note was published to his private Claude artifacts on the same day.
