...
Use separate predicates: brief exists, tasks have been created, and at least one task is ready. A folder or parent issue is a location, not evidence that planning is complete. See [planning entry](../plan/plan-skill/01-make-planning-repeatable.md) and [routing](../plan/atlas-skill/02-route-from-current-state.md).
...
Resolve the repository, fetch, select or resume the branch, and verify the permitted starting state before recording progress. Treat the local change and external claim as a recoverable sequence. See [GitHub start](../plan/build-skill/02-make-github-start-recoverable.md).
...
Distinguish active work, waiting for an answer, ready for review, and changes requested through an explicit transition record. Define argument validation and resumption. See [build retry](../plan/build-skill/01-support-resume-and-fixes.md).
...
Bind delivery to an artifact revision, review to that delivery and the brief revision, and invalidate approval when either changes. Use file hashes or explicit revisions when Git is absent. See [review evidence](../plan/review-skill/01-bind-reviews-to-deliveries.md).
...
Use a handoff as a candidate action, validate its target, and prefer current task evidence. Give handoffs stable identity and deterministic ordering. Keep the historical note immutable. See [handoff](../plan/side-skills/02-make-handoffs-resumable.md) and [routing](../plan/atlas-skill/02-route-from-current-state.md).
...
Adopt existing context by classifying its content into the five homes, retaining provenance, verifying coverage and inbound links, and treating source removal as a separately reviewable migration step. Deduplicate instruction-block writes by resolved path. See [bootstrap recovery](../plan/atlas-skill/01-recover-partial-initialization.md).
...
Capture a starting diff, track owned changes, and verify the exact staged diff before commit. Retain task and artifact paths in the receipt. Handoff should preserve the user's uncommitted work as information, with a concrete authorized scope for any commit. See [owned writes](../plan/build-skill/03-preserve-owned-changes.md) and [handoff](../plan/side-skills/02-make-handoffs-resumable.md).
...
Define Atlas's state first, then specify the evidence the GitHub adapter reads. Ownership and lifecycle should have distinct meanings. Canceled work should have an explicit disposition rather than releasing a dependency as successful delivery. See [contract](../plan/five-things/01-settle-workflow-contract.md) and [GitHub completion](../plan/review-skill/02-verify-github-completion.md).
...
Choose one compatible design: keep the public router manual, expose selected worker skills for agent invocation, or supply a separately authorized orchestration procedure with its own supported entry point. Test the choice in each advertised harness; changing metadata alone does not establish safe transitions. See [go and invocation](../plan/atlas-skill/03-align-go-with-invocation.md).
...
Require a changed task or delivery revision after each automatic step. Stop on a pending human action, failed prerequisite, unchanged state, or a user-specified run limit. Persist the reason and exact resumable target. See [go and invocation](../plan/atlas-skill/03-align-go-with-invocation.md).
...
Validate both dependency graphs, with actionable cycle paths and missing targets. Render from validated data. Scope the meaning of Needs to decision readiness; task dependencies remain the build order. See [map validation](../plan/map-skill/01-validate-graphs-and-views.md) and [planning](../plan/plan-skill/01-make-planning-repeatable.md).
...
Define a shared resolver for stable part ids and their owning file. Specify parent/child ownership, summary edges, existing task retention, and rename references. Separate zooming a view from changing the decomposition of work. See [part migration](../plan/map-skill/02-preserve-split-part-identity.md).
...
Version the portable format, define the owner of each invariant, and provide migrations as inspectable changes. Treat templates as distributable sources and project formats as versioned instances with documented overrides. Preserve backward reading during rollout. See [format migration](../plan/five-things/02-version-and-migrate-formats.md).
...
Require an actual planned scope, current accepted evidence for its required tasks, and a proportional final check of the part's outcome. Reopen explicitly when new accepted scope invalidates completion. Distinguish delivery from release or publication when the domain needs it. See [part completion](../plan/review-skill/03-check-part-outcomes.md).
...
Make the checklist express behavioral obligations and justify exceptions explicitly: initialization has different prerequisites, a status report has five fields rather than five physical lines, and a failure carries the information needed to resume. Positive prose is a useful editing preference, not proof that a skill is safe or clear. See [authoring contract](../plan/packaging/01-align-authoring-and-installation.md).
...
Keep the example labeled as illustrative until it contains verifiable work. Add fixture variants for each lifecycle boundary and test installation in an isolated profile. See [examples](../plan/packaging/02-build-realistic-fixtures.md) and [installation](../plan/packaging/01-align-authoring-and-installation.md).
...
|---|---|---|
| P1 | [five-things/01: settle the workflow contract](../plan/five-things/01-settle-workflow-contract.md) | F03-F04, F09, F14, F17, F21 |
| P1 | [atlas-skill/01: recover partial initialization](../plan/atlas-skill/01-recover-partial-initialization.md) | F06-F07 |
| P1 | [atlas-skill/02: route from current state](../plan/atlas-skill/02-route-from-current-state.md) | F01, F03, F05, F13 |
| P1 | [atlas-skill/03: align go with invocation](../plan/atlas-skill/03-align-go-with-invocation.md) | F12-F13 |
| P1 | [plan-skill/01: make planning repeatable](../plan/plan-skill/01-make-planning-repeatable.md) | F01, F15, F23 |
| P1 | [plan-skill/02: preserve replan history](../plan/plan-skill/02-preserve-replan-history.md) | F11, F20 |
| P1 | [build-skill/01: support resume and fixes](../plan/build-skill/01-support-resume-and-fixes.md) | F03-F04 |
| P1 | [build-skill/02: make GitHub start recoverable](../plan/build-skill/02-make-github-start-recoverable.md) | F02, F09, F23 |
| P1 | [build-skill/03: preserve owned changes](../plan/build-skill/03-preserve-owned-changes.md) | F08 |
| P1 | [review-skill/01: bind reviews to deliveries](../plan/review-skill/01-bind-reviews-to-deliveries.md) | F04, F22 |
| P1 | [review-skill/02: verify GitHub completion](../plan/review-skill/02-verify-github-completion.md) | F09-F10, F23 |
| P1 | [side-skills/02: make handoffs resumable](../plan/side-skills/02-make-handoffs-resumable.md) | F05, F08, F26 |
| P2 | [five-things/02: version and migrate formats](../plan/five-things/02-version-and-migrate-formats.md) | F18, F26 |
| P2 | [five-things/03: evaluate tracker and Git policy](../plan/five-things/03-evaluate-tracker-and-git-policy.md) | F24 |
| P2 | [interview-skill/01: sharpen interview decisions](../plan/interview-skill/01-sharpen-interview-decisions.md) | F17, F19-F20 |
| P2 | [interview-skill/02: preserve brief revisions](../plan/interview-skill/02-preserve-brief-revisions.md) | F20-F21 |
| P2 | [map-skill/01: validate graphs and views](../plan/map-skill/01-validate-graphs-and-views.md) | F15, F19 |
| P2 | [map-skill/02: preserve split part identity](../plan/map-skill/02-preserve-split-part-identity.md) | F16 |
| P2 | [review-skill/03: check part outcomes](../plan/review-skill/03-check-part-outcomes.md) | F21 |
| P2 | [side-skills/01: make prototypes conclusive](../plan/side-skills/01-make-prototypes-conclusive.md) | F25-F26 |
| P2 | [packaging/02: build realistic fixtures](../plan/packaging/02-build-realistic-fixtures.md) | F28 and behavioral evidence |
| P2 | [packaging/03: run release acceptance](../plan/packaging/03-run-release-acceptance.md) | Both trackers, advertised harnesses, real project |
| P3 | [packaging/01: align authoring and installation](../plan/packaging/01-align-authoring-and-installation.md) | F27-F28 |
