...
Atlas baseline: `472ca9508bdf896dbe000c0f66d81157a4d10afc`. There are **nine executable operations**, including `setup-atlas`, in draft 0.2. Some older package descriptions still count eight. Keep three evidence levels separate:
...
| Small work | One task can be added under an existing brief, but the draft still has a confirmation step; building and review are separate invocations. | The router sends work that fits one session directly to implementation without a separate spec and decomposition. | **Matt's route is simpler.** Finish Atlas's accepted direct route: settled brief, one task, necessary work and checks. The benefit of an extra interview or planning round must be a remaining decision. |
...
1. **Finish the cold run of patched 0.2.** Use [work-skills/06](../plan/work-skills/06-patch-the-draft.md) to see which remaining friction is real before expanding the contracts. Keep the difference between agent rehearsal and the person's acceptance visible.
...
6. **Publish demonstrated claims.** Complete [cold-context acceptance](../plan/validation/03-run-cold-context-acceptance.md), align user documentation, and only then assess a tagged release. Additional commands and automatic orchestration can wait for an observed need.
